...
path_output = f'D:/___DEEPFACES_SERVER/zar3i_site_updates/{dir_update}/4_communesPNG/Conventional_seeding/'
path_NDVI = f'D:/___DEEPFACES_SERVER/zar3i_site_updates/{dir_update}/2_merge/NDVI.tif'

raster_layer_NDVI = QgsRasterLayer(path_NDVI, 'ndvi')
fieldsLayer = QgsProject.instance().mapLayersByName('agriculture_segmentationfield')[0]
...
for commune in communes:
    ...
    fieldsLayer_reprojected_dissolved_selected = processing.run("native:dissolve", {
    ...
    NDVI_clip_fields_int16 = processing.run("gdal:rastercalculator", {
        'INPUT_A':raster_layer_NDVI_fields.source(),
        'BAND_A':1,
        'INPUT_B':None,
        'BAND_B':None,
        'INPUT_C':None,
        ...
        'FORMULA':'A*100',
        'NO_DATA':0,
        'PROJWIN':raster_layer_NDVI_commune.extent(),
        'RTYPE':1,
        'OPTIONS':'',
        'EXTRA':'',
        'OUTPUT':path_output + 'id_' + str(commune['id']) + '.tif'
    })['OUTPUT']
    raster_layer_NDVIint16 = QgsRasterLayer(NDVI_clip_fields_int16,'id_' + str(commune['id']))
    ...
    colorTableNDVI = {
        ...
        9:{'minValue':  min_+(9*interval)/10, 'maxValue':max_, 'R': 26, 'G':150, 'B': 65}
    }
